...
async function refreshVolume(kind,level=null){
  ...
  try{const result=await invoke('audio_volume',{kind,device,level});if(sequence!==control.sequence||device!==volumeDevice(kind)||control.dragging)return;$(kind+'-volume').value=result.level;control.available=true;$(kind+'-level').textContent=result.level+'%';const notices=[];if(result.level>100)notices.push(fill('GAIN_APPLIED',result.level+'%'));if(kind==='microphone'&&state.microphone_fallback)notices.push(t('MICROPHONE_SILENT'));if(result.muted)notices.push(t('MICROPHONE_MUTED'));$(kind+'-volume-status').textContent=notices.join(' / ');}
