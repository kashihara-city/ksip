...
番号・転送先・ダイヤル先には `sip:` で始まる完全なSIP URI（`<sip:61@pbx.example>` のように山括弧で囲んでも可）も書けます。BLFはその番号の `dialog` イベントを購読して得ます。SIPサーバー側でその番号にBLFが公開されていないと「状態不明」のままです（Asteriskでは `hint` が要ります）。
...
| `button_<n>_kind`     | ボタン n の機能。`transfer`＝転送、`dial`＝ダイヤル（BLF付き）、`park`＝パーク保留（BLF付き）、`open`＝リンクを開く                                    | 空（ボタンを出さない）      |
