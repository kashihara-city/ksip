...
- 録音：`recordings\`（最初に録音したときに作ります。画面の「保存先」で開けます）。ファイル名は録音を始めた日時と相手の番号で、`2026-09-23_14-30-12_1002.wav` のようになります。自動録音は通話中の相手を追いかけるので、転送などで1つのファイルに複数の通話が入ることがあります。通話履歴には、録音ファイルが残っている通話に「録音を再生」が出て、Windowsの既定の再生アプリで開きます。
