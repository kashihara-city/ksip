...
通話が終わるたびに履歴に1行足します。つながらなかった通話は、通話時間の代わりに理由を赤字で出します。発信では SIP の応答に応じて「話中」（486）「番号なし」（404）「不在」（480/503）「拒否」（603）「応答なし」（408）、こちらから途中で切った場合は「中止」、それ以外の失敗は「不通」です。着信では、出なかったものが「不在着信」、同じ着信で鳴っていた他の電話が取ったものが「他で応答」、着信拒否中に断ったものが「着信拒否」です。「他で応答」は失敗ではないので赤字にせず、通話時間と同じ色で出します。「他で応答」は、PBX が呼び出しの取り消し（CANCEL）に理由（RFC 3326 の `Reason` ヘッダーで `SIP;cause=200` か `Q.850;cause=26`）を付けてくる場合に出ます。付けてこない PBX では「不在着信」になります。発信がつながらなかったときは画面にも「つながりませんでした: 話中」のように出て、次の操作が通ると消えます。
