...
| `button_<n>_title`    | カスタムボタン n（1〜6。7〜30は拡張ボタン）のタイトル。40文字以内。空なら番号を出す                                                                                     | 番号を出す                  |
| `button_<n>_kind`     | ボタン n の機能。`transfer`＝転送、`dial`＝ダイヤル（BLF付き）、`park`＝パーク保留（BLF付き）、`open`＝リンクを開く、`dnd`＝着信拒否                                    | 空（ボタンを出さない）      |
