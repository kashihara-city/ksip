...
設定画面で最大6つのボタンを定義でき、設定したものだけが画面に並びます。各ボタンはタイトル・機能・番号を持ちます。

| 機能         | 押したとき                                                                                                                      |
| ------------ | ------------------------------------------------------------------------------------------------------------------------------- |
| 転送         | 通話中の相手を番号へブラインド転送します                                                                                        |
| 短縮ダイヤル | 番号へ発信します。番号の使用状況（BLF）を表示します                                                                             |
| 保留         | BLFが空きなら通話を番号へブラインド転送し、使用中なら番号へ発信して取ります。転送先がBLFの番号と違うPBXでは「転送先」に書きます |
| リンクを開く | 番号欄に書いたURL（`http://` か `https://`）をブラウザで開きます                                                                |

番号と転送先には `sip:` で始まる完全なSIP URI（`<sip:61@pbx.example>` のように山括弧で囲んでも可）も書けます。BLFはその番号の `dialog` イベントを購読して得ます。SIPサーバー側でその番号にBLFが公開されていないと「状態不明」のままです（Asteriskでは `hint` が要ります）。
...
単一exeで動作しますが、運用上保存しておいたほうが良いファイル・フォルダは、`ksip.exe` と同じ場所に配置します。フォルダごとコピーすれば移動できます。

- 通話履歴：exeと同じ場所の `call-history.jsonl`（1行に1件のJSON。通話が終わるたびに追記し、2000件を超えると直近1000件に書き直します。画面の「クリア」で消去できます。0.0.5以前の `call-history.json` は最初の起動で取り込み、その後は使わないので消してかまいません）
- ログ：exeと同じ場所の `ksip-log.jsonl`（1行に1件のJSON。新しい行を1秒に1回まとめて追記し、2000行を超えると直近1000行に書き直します。KSIPを起動し直しても消えず、画面の「クリア」で消去できます。画面に出るのは今回の起動分だけです。0.0.5以前の `ksip-log.json` は使わなくなったので消してかまいません）
- 録音：exeと同じ場所の `recordings/`（最初に録音したときに作ります）。ファイル名は録音を始めた日時と相手の番号で、`2026-09-23_14-30-12_1002.wav` のようになります。自動録音は通話中の相手を追いかけるので、転送などで1つのファイルに複数の通話が入ることがあります。通話履歴には、録音ファイルが残っている通話に「録音を再生」が出て、Windowsの既定の再生アプリで開きます。
- 通知アイコン : exeと同じ場所の`ksip-notification.ico`（起動時に自動で作成します）
...
| `button_<n>_title`    | カスタムボタン n（1〜6）のタイトル。40文字以内。空なら番号を出す                                                                                     | 番号を出す                  |
| `button_<n>_kind`     | ボタン n の機能。`transfer`＝転送、`dial`＝短縮ダイヤル（BLF付き）、`park`＝保留（BLF付き）、`open`＝リンクを開く                                    | 空（ボタンを出さない）      |
| `button_<n>_number`   | ボタン n の番号。数字と `*` `#`（先頭に `+` も可）で30文字以内、または `sip:` で始まるSIP URI。`open` では `http://` か `https://` のURL             | 空                          |
| `button_<n>_transfer` | `park` のときだけ。転送先がBLFの番号と違う場合の転送先。空なら `button_<n>_number` へ転送                                                            | 番号と同じ                  |
