...
void PlayoutDestructor(void *arg) {
  ...
  if (state == g_active_playout) {
    if (state->started && g_audio) ksip_audio_stop_playout(g_audio);
...
void SourceDestructor(void *arg) {
  ...
  if (state == g_active_source) {
    if (state->started && g_audio) ksip_audio_stop_recording(g_audio);
...
int AllocatePlayout(struct auplay_st **out, const struct auplay *,
  ...
  if (g_active_playout) {
    ksip_audio_stop_playout(g_audio);
  ...
  info("ksip_audio: WebRTC ADM playout started\n");
  return 0;
}
int AllocateSource(struct ausrc_st **out, const struct ausrc *,
  ...
  if (!Valid(p)) return ENOTSUP;
  if (g_active_source) {
    ksip_audio_stop_recording(g_audio);
  ...
  if (result) {
    ...
    state->started = true;
    info("ksip_audio: WebRTC ADM recording and APM started\n");
  }
