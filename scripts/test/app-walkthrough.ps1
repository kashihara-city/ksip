...
try {
    ...
    # screen and kept in the history in place of a duration. A PBX that answers
    # such a call with an announcement instead (3CX) says so in lab.json, and
    # there is nothing to see then.
    if((Get-KsipLab).features.unassigned_rejected -eq $false){
        'NOTE: 未割当番号を PBX が案内で応答するので、つながらない発信の理由の表示は確かめない（lab.json の features.unassigned_rejected が false）'
    }else{
    ...
    # The row appears once the recording has been closed and the history
    # re-read, a moment after the call ends, so it is waited for.
    foreach($label in '録音を再生','ファイルの場所を開く'){
        ...
        while(!(Wait-Id 'call-history').FindFirst([Windows.Automation.TreeScope]::Descendants,$wanted)){
            if([DateTime]::UtcNow -ge $end){throw "The recorded call has no $label button"}
